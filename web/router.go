...
func (r *router) RegisterHandler() {
	...
	r.muxr.Handle("/friend/list", chain.ThenFunc(http_handler.Friend.ListFriend)).Methods("GET").Queries("aid", "{aid}").Queries("uid", "{uid}")
	r.muxr.Handle("/group/create", chain.ThenFunc(http_handler.Group.Create)).Methods("POST")
	r.muxr.Handle("/group/info", chain.ThenFunc(http_handler.Group.Info)).Methods("GET").Queries("gid", "{gid}")
	r.muxr.Handle("/group/update", chain.ThenFunc(http_handler.Group.Update)).Methods("PUT")
	r.muxr.Handle("/group/users", chain.ThenFunc(http_handler.Group.Users)).Methods("GET").Queries("gid", "{gid}")
	...
	r.muxr.Handle("/group/user/kick", chain.ThenFunc(http_handler.Group.KickUser)).Methods("DELETE").Queries("gid", "{gid}").Queries("uid", "{uid}")
