...
func (r *router) RegisterHandler() {
	...
	r.muxr.Handle("/group/info", chain.ThenFunc(handler.Group.Info)).Methods("GET").Queries("aid", "{aid}").Queries("gid", "{gid}")
	...
	r.muxr.Handle("/group/user/update", chain.ThenFunc(handler.Group.UpdateUser)).Methods("PUT")
	r.muxr.Handle("/group/user/groups", chain.ThenFunc(handler.Group.Groups)).Methods("GET").Queries("aid", "{aid}").Queries("uid", "{uid}")
	r.muxr.Handle("/group/user/kick", chain.ThenFunc(handler.Group.KickUser)).Methods("DELETE").Queries("aid", "{aid}").Queries("gid", "{gid}").Queries("uid", "{uid}")
